...
$('#service_selector').change(function(){   
	var opt = $(this).val();
	if (opt == "aws") {
		$("#size_selector").html('<option value="Micro" selected="selected" >Micro</option><option value="medium">Medium</option><option value="large">Large</option>');
		//$('#size_selector').selectpicker('refresh');
		$("#region_selector").html('<option value="asia-east1-b">Tokyo</option>'+
                                   '<option value="europe-west1-b">N. Carolina</option>'+
                                    '<option value="us-central1-b">Sydney</option>'+
                                    '<option value="us-east1-b">Ireland</option>');
		$("#myModalLabel").text("Pleae Input AWS Key");
		$("#keyForm").html('<div class="form-group"  style="color:black;">'+
'                                        <label id="publicKeyLabel" style="color:black;">AWS Public Key</label>'+
'                                       <input id="awsPublicKey" type="email" class="form-control"'+
'                                        placeholder="Public Key"  style="color:black;"/>'+
'                                    </div>'+
'                                    <div class="form-group" style="color:black;">'+
'                                        <label for="publicKeyLabel" style="color:black;">AWS Secret Key</label>'+
'                                        <input id="awsSecretKey" type="password" class="form-control"'+
'                                        placeholder="Secret Key"  style="color:black;"/>'+
'                                   </div>');
		$("#a").attr("href", "https://console.aws.amazon.com/console/home")
// 		$(".modal-footer").html('<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>' +
// '             <a href="https://twitter.com/SBootstrap" class="btn btn-default"> '+
// '             <i class="fa fa-amazon fa-fw"></i>'+
// '             <span class="network-name">Create Your Keys</span></a>'+
// '             <a id="creatButton" data-target=type="button" class="btn btn-primary" data-dismiss="modal" href="#contact"> '+
// '             Submit and Create Instance '+
// '             </a>');
...
'                                        <input id="googleApiKey" type="password" class="form-control"placeholder="API Key"/>'+
'                                   </div>' );
		$("#a").attr("href", "https://appengine.google.com/")
// 		$(".modal-footer").html('<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>' +
// '             <a href="https://twitter.com/SBootstrap" class="btn btn-default"> '+
...
// '             </a>');
		...
		$("#keyForm").html('<div class="form-group"  style="color:black;">'+
'                                        <label id="publicKeyLabel" style="color:black;">Aliyun Public Key</label>'+
'                                        <input id="aliyunPublicKey" type="email" class="form-control"'+
'                                        placeholder="Public Key"/>'+
'                                    </div>'+
'                                    <div class="form-group" style="color:black;">'+
'                                        <label for="publicKeyLabel" style="color:black;">Aliyun Secret Key</label>'+
'                                        <input id="aliyunSecretKey" type="password" class="form-control"'+
'                                        placeholder="Secret Key"/>'+
'                                    </div>');
		$("#a").attr("href", "https://account.aliyun.com/register/intl_register.htm?spm=0.0.0.0.oSZUEA&lang=en")
...
function initializeApi() {
  gapi.client.load('compute', API_VERSION,function() {
		...
		setTimeout(function(){ addAccessConfig(); }, 40000);
		setTimeout(function(){ getInstance(); }, 60000);	
...
'                        </div>';
		...
		buttonListenerFresh();
			...
			window.alert(apiRequestName + "successful");
		 //newWindow = window.open(apiRequestName, '', 'width=600, height=600, scrollbars=yes');
   		 //newWindow.document.write('<h1>' + apiRequestName + '</h1> <br />' + '<pre>' + JSON.stringify(resp.result, null, ' ') + '</pre>');
	}else{
		    window.alert(apiRequestName  + "successful");
